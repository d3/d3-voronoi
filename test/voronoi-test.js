...
tape("voronoi.extent([[x1, y1], [x2, y2]]) sets the specified extent", function(test) {
  ...
  test.deepEqual(asArray(v([[200, 200], [760, 300]])), [
...
tape("voronoi.x(x) sets the specified x-accessor", function(test) {
  ...
  test.deepEqual(asArray(v([{x: 200, 1: 200}, {x: 760, 1: 300}])), [
...
tape("voronoi.y(y) sets the specified y-accessor", function(test) {
  ...
  test.deepEqual(asArray(v([{0: 200, y: 200}, {0: 760, y: 300}])), [
    [[-178046.78571428574, 1e6], [179096.07142857145, -1e6], [-1e6, -1e6], [-1e6, 1e6]],
    [[179096.07142857145, -1e6], [-178046.78571428574, 1e6], [1e6, 1e6], [1e6, -1e6]]
  ]);
  test.end();
...
tape("voronoi.x(x) allows the specified x-accessor to be a constant", function(test) {
  ...
  test.equal(v.x(42), v);
  test.equal(v.x(), 42);
  test.deepEqual(asArray(v([200, 760])), [
...
tape("voronoi(points) has an implicit extent of [[-1e6, -1e6], [1e6, 1e6]] by default", function(test) {
  var x0 = Infinity,
      ...
      y1 = -Infinity,
      v = voronoi.voronoi();
  v([[200, 200], [760, 300]]).forEach(function(cell) {
  ...
  test.equal(y0, -1e6);
  test.equal(y1, 1e6);
  test.deepEqual(asArray(v([[50, 50]])), [[[-1e6, 1e6], [1e6, 1e6], [1e6, -1e6], [-1e6, -1e6]]]);
  test.ok(v([[50, 50]]).every(function(polygon) { return polygonArea(polygon) > 0; }));
  test.end();
});
...
tape("voronoi(points) can separate two points with a horizontal line", function(test) {
  test.deepEqual(asArray(voronoi.voronoi()([[0, -100], [0, 100]])), [
...
tape("voronoi(points) can separate two points with a vertical line", function(test) {
  test.deepEqual(asArray(voronoi.voronoi()([[100, 0], [-100, 0]])), [
...
tape("voronoi(points) can separate two points with a diagonal line", function(test) {
  test.deepEqual(asArray(voronoi.voronoi()([[-100, -100], [100, 100]])), [
...
tape("voronoi(points) can separate two points with an arbitrary diagonal", function(test) {
  test.deepEqual(asArray(voronoi.voronoi()([[-100, -100], [100, 0]])), [
...
tape("voronoi(points) can handle three collinear points", function(test) {
  test.deepEqual(asArray(voronoi.voronoi()([[-100, -100], [0, 0], [100, 100]])), [
